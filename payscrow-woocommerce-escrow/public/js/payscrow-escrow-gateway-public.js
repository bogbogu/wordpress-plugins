(function( $ ) {
	...
	$(function() {
		...
		$('#apply-escrow-code').on('click', function(e) {
			...
			if (!code) {
				$messageArea.html('<p class="error">' + payscrow_wc_escrow_params.i18n_error + 'Please enter an escrow code.</p>');
				return;
			}
			...
			$.ajax({
				type: 'POST',
				url: payscrow_wc_escrow_params.ajax_url,
				data: {
					action: 'apply_escrow_code',
					escrow_code: code,
					security: payscrow_wc_escrow_params.apply_escrow_nonce
				},
				success: function(response) {
					if (response.success) {
						...
						$messageArea.html('<p class="error">' + payscrow_wc_escrow_params.i18n_error + response.data.message + '</p>');
					}
				},
				error: function() {
					$messageArea.html('<p class="error">' + payscrow_wc_escrow_params.i18n_error + 'Could not connect to the server.</p>');
				},
				complete: function() {
		...
		$('body').on('payment_method_selected', function() {
			...
			if (selectedMethod === 'payscrow_wc_escrow') {
