 ...
 */
class Klassified_Escrow_Gateway_Payment
{
     ...
     * @since    1.0.0
     * @access   private
     * @var      Klassified_Escrow_Gateway_API    $api    API instance
     */
    private $api;
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        $this->settings = get_option('klassified_escrow_gateway_settings', array());
        $this->debug = isset($this->settings['debug_mode']) && $this->settings['debug_mode'] === 'yes';
        $this->api = new Klassified_Escrow_Gateway_API();
    ...
    {
        $gateways[] = 'WC_Gateway_Klassified_Escrow';
        return $gateways;
    }
    ...
    private function init_gateway_class()
    {
        ...
        require_once plugin_dir_path(dirname(__FILE__)) . 'includes/gateways/class-wc-gateway-klassified-escrow.php';
    }
    ...
    public function process_escrow_payment($order_id, $posted_data, $order)
    {
        ...
        if ($order->get_payment_method() !== 'klassified_escrow') {
            return;
        }
        ...
        if (is_wp_error($api_data)) {
            ...
            $order->add_order_note(__('Escrow payment failed: ', 'klassified-escrow-gateway') . $error_message);
        ...
        if (is_wp_error($response)) {
            ...
            $admin_message = __('Escrow payment API error: ', 'klassified-escrow-gateway') . $error_message;
            ...
            $user_message = __('Payment error: ', 'klassified-escrow-gateway');

            // Add specific troubleshooting suggestions based on error type
            if (strpos($error_message, 'HTTP 400') !== false) {
                $user_message .= __('The payment could not be processed due to an issue with the request. This might be due to missing information or an API configuration issue. Please try again or contact support.', 'klassified-escrow-gateway');
            } elseif (strpos($error_message, 'HTTP 401') !== false || strpos($error_message, 'HTTP 403') !== false) {
                $user_message .= __('Authentication failed with the payment processor. Please contact the store administrator to verify the API settings.', 'klassified-escrow-gateway');
            } elseif (strpos($error_message, 'connect') !== false) {
                $user_message .= __('Could not connect to the payment processor. Please try again later or use a different payment method.', 'klassified-escrow-gateway');
            } else {
        ...
        if (empty($payment_link)) {
            $error_message = __('No payment link returned from escrow service. Please check API configuration.', 'klassified-escrow-gateway');
            $order->add_order_note(__('Escrow payment error: ', 'klassified-escrow-gateway') . $error_message);
            wc_add_notice(__('Payment error: ', 'klassified-escrow-gateway') . $error_message, 'error');
        ...
        if (!empty($transaction_id)) {
            ...
            $order->save();
            $order->add_order_note(sprintf(__('PayScrow Transaction ID: %s', 'klassified-escrow-gateway'), $transaction_id));
        ...
        $order->update_status('pending', __('Awaiting escrow payment', 'klassified-escrow-gateway'));
    ...
    private function build_api_request_data($order)
    {
        ...
        if (empty($admin_account_name) || empty($admin_account_number) || empty($admin_bank_code)) {
            return new WP_Error('invalid_admin_account', __('Admin account details are not properly configured.', 'klassified-escrow-gateway'));
        }
        ...
        foreach ($order->get_items() as $item_id => $item) {
            ...
            $vendor_account_name = get_user_meta($vendor_id, 'klassified_account_name', true);
            $vendor_account_number = get_user_meta($vendor_id, 'klassified_account_number', true);
            $vendor_bank_code = get_user_meta($vendor_id, 'klassified_bank_code', true);
            ...
            if (empty($vendor_account_name) || empty($vendor_account_number) || empty($vendor_bank_code)) {
                ...
                return new WP_Error('invalid_vendor_account', sprintf(__('Vendor "%s" has incomplete bank details.', 'klassified-escrow-gateway'), $vendor_name));
            }
        ...
        if (empty($admin_account_name) || empty($admin_account_number) || empty($admin_bank_code)) {
            return new WP_Error('invalid_admin_account', __('Admin bank account details are missing. Please configure them in the plugin settings.', 'klassified-escrow-gateway'));
        }

        if (empty($items)) {
            return new WP_Error('no_items', __('No valid items found in order.', 'klassified-escrow-gateway'));
        }
        ...
        // Build webhook URL
        $webhook_url = get_rest_url(null, 'klassified/v1/webhook');

        // Merchant info (site info) - ensure we have values
        $merchant_name = get_bloginfo('name');
        if (empty($merchant_name)) {
            $merchant_name = 'Klassified Mall'; // Fallback to a default value if empty
        }
    ...
    public function add_escrow_code_field()
    {
        ...
        if (WC()->session && WC()->session->get('chosen_payment_method') === 'klassified_escrow') {
            wc_get_template(
            ...
            wp_enqueue_script('klassified-escrow-public-js');
        }
    }
    ...
    public function apply_escrow_code_discount($cart)
    {
        ...
        if (isset($escrow_code_data['isValid']) && $escrow_code_data['isValid'] && isset($escrow_code_data['amount'])) {
            ...
            if ($discount_amount > 0) {
                $cart->add_fee(
                    sprintf(__('Escrow Code Discount (%s)', 'klassified-escrow-gateway'), $escrow_code),
                    -$discount_amount,
                    false
                );
            }
    ...
    public function apply_escrow_code_ajax()
    {
        ...
        if (empty($escrow_code)) {
            wp_send_json_error(array(
                'message' => __('Please enter an escrow code.', 'klassified-escrow-gateway')
            ));
            return;
        }
        ...
        if (!isset($response['isValid']) || !$response['isValid']) {
            wp_send_json_error(array(
                'message' => __('Invalid escrow code.', 'klassified-escrow-gateway')
            ));
            return;
        }
        ...
        wp_send_json_success(array(
            'message' => sprintf(__('Escrow code applied. You will receive a discount of %s.', 'klassified-escrow-gateway'), wc_price($response['amount'])),
    ...
    private function log($message, $force_log = false)
    {
        ...
        if ($force_log) {
            error_log("KLASSIFIED ESCROW: {$message}");
        }
    }
}
