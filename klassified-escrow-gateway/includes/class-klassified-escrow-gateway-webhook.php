 ...
 */
class Klassified_Escrow_Gateway_Webhook {
    ...
    public function register_webhook_endpoint() {
        register_rest_route('klassified/v1', '/webhook', array(
            'methods'  => 'POST',
    ...
    public function process_webhook($request) {
        ...
        if (!empty($escrow_code)) {
            ...
            $order->save();
            $order->add_order_note(sprintf(__('PayScrow escrow code: %s', 'klassified-escrow-gateway'), $escrow_code));
        }
        ...
        switch ($status) {
            ...
            case 'InProgress':
                ...
                $order->update_status('processing', __('Payment confirmed and in escrow via PayScrow.', 'klassified-escrow-gateway'));
                break;
                
            case 'Completed':
                // Customer has been satisfied and released escrow code
                $order->add_order_note(__('Customer has released the escrow code via PayScrow.', 'klassified-escrow-gateway'));
                $order->update_status('completed', __('Transaction completed in PayScrow.', 'klassified-escrow-gateway'));
                break;
                
            case 'Finalized':
                // All parties settled and transaction closed
                $order->update_status('completed', __('Transaction finalized in PayScrow.', 'klassified-escrow-gateway'));
                break;
                
            case 'Pending':
                // Transaction created but no payment yet
                $order->update_status('pending', __('Awaiting payment via PayScrow.', 'klassified-escrow-gateway'));
                break;
                
            case 'Cancelled':
                // Transaction cancelled
                $order->update_status('cancelled', __('Transaction cancelled in PayScrow.', 'klassified-escrow-gateway'));
                break;
                
            default:
                // Unknown status
                $order->add_order_note(sprintf(__('PayScrow status updated to: %s', 'klassified-escrow-gateway'), $status));
                break;
        }
        
        // Handle dispute status separately (it's a boolean flag)
        if (isset($transaction_status['inDispute']) && $transaction_status['inDispute']) {
            $order->update_status('on-hold', __('Order has been disputed in PayScrow.', 'klassified-escrow-gateway'));
            $order->add_order_note(__('Order dispute opened in PayScrow.', 'klassified-escrow-gateway'));
        }
